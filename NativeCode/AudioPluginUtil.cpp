...
#define ENABLE_TESTS ((PLATFORM_WIN || PLATFORM_OSX) && 1)
    ...
    HistoryBuffer::HistoryBuffer()
        : length(0), writeindex(0), data(NULL)
    {
    }
        ...
        const char *description)
    {
        ...
        definition.paramdefs[enumvalue].description = (description != NULL) ? strnew(description) : (name != NULL) ? strnew(name)
                                                                                                                   : NULL;
...
#define NAP_UNITTEST(name)                     \
    struct NAP_Test_##name                     \
    {                                          \
        NAP_Test_##name(const char *testname); \
    };                                         \
    ...
    NAP_UNITTEST(Accuracy)
    {
        for (int test = 0; test < 2; test++)
        {
            ...
            for (int b = 4; b <= 20; b++)
            {
                ...
                {
                    float err, diff;
                    diff = test1[n].re - test2[n].re;
                    err = fabsf(diff);
                    NAP_CHECK(err < errtol);
                    errsum += err;
                    if (err > maxerr)
                        maxerr = err;
                    rms += diff * diff;
                    diff = test1[n].im - test2[n].im;
                    err = fabsf(diff);
                    NAP_CHECK(err < errtol);
                    errsum += err;
                    if (err > maxerr)
